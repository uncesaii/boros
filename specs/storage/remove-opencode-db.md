# Remove `packages/boros/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/boros/src/storage/db.ts` module.
...
Production imports from `packages/boros/src/storage/db.ts` are concentrated in 22 source files:

- `packages/boros/src/account/repo.ts`
- `packages/boros/src/cli/cmd/db.ts`
- `packages/boros/src/cli/cmd/import.ts`
- `packages/boros/src/cli/cmd/stats.ts`
- `packages/boros/src/control-plane/workspace.ts`
- `packages/boros/src/index.ts`
- `packages/boros/src/node.ts`
- `packages/boros/src/permission/index.ts`
- `packages/boros/src/project/project.ts`
- `packages/boros/src/server/projectors.ts`
- `packages/boros/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/boros/src/server/shared/fence.ts`
- `packages/boros/src/session/message-v2.ts`
- `packages/boros/src/session/projectors.ts`
- `packages/boros/src/session/prompt.ts`
- `packages/boros/src/session/session.ts`
- `packages/boros/src/session/todo.ts`
- `packages/boros/src/share/share-next.ts`
- `packages/boros/src/storage/db.ts`
- `packages/boros/src/sync/index.ts`
- `packages/boros/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy opencode database wrapper; `packages/boros/src/storage/db.ts` has been deleted.
...
All migration groups are complete or superseded. `packages/boros/src/storage/db.ts` has been deleted.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/boros`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/boros/src`
- `bun typecheck` from `packages/boros`
- Relevant package tests from `packages/boros`, not the repo root
