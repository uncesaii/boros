...
`packages/boros/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/boros`,
`packages/cli`, or `@boros-ai/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/boros`.
- Do not replace private `packages/boros` imports with `@boros-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@boros-ai/tui`.
...
```text
packages/boros/src/cli/cmd/tui
```

Its private dependency on `packages/boros` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/boros/src/*`.
...
Status: Completed. The private `@boros-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@boros-ai/tui`.
...
- The package has no dependency on `opencode`, `@boros-ai/cli`, or
  `@boros-ai/core`.
...
primitives now live in `@boros-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@boros-ai/tui/runtime`. Movable app, component,
...
`@boros-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/boros` initially.
...
canonical in `@boros-ai/tui`. Configured references resolve through the new
...
`@boros-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/boros`.
...
- The package has no imports from `packages/boros`, `packages/cli`, or
  `@boros-ai/core`.
...
public `@boros-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/boros`.
- Keep the legacy embedded worker and server startup in `packages/boros`.
- Change those adapters to load config, create transport inputs, and call the
  public `@boros-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/boros`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@boros-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@boros-ai/core`, or either
  executable package.
...
cd packages/boros && bun typecheck
...
rg '@boros-ai/core|packages/boros|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/boros/src packages/boros/test
```
...
- Build the current-platform `packages/boros` binary.
