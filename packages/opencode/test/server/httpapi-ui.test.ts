  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("BOROS_SERVER_PASSWORD", original.envPassword)
        restoreEnv("BOROS_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            BOROS_SERVER_PASSWORD: input?.password,
            BOROS_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
