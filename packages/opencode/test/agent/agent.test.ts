...
it.instance("build agent has correct default properties", () =>
  Effect.gen(function* () {
    const build = yield* load((svc) => svc.get("operator"))
...
it.instance("plan agent denies edits except .boros/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".boros/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
    ...
    }),
  {
    config: {
      agent: {
        operator: { prompt: "Custom system prompt" },
      },
    },
  },
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".boros", "skill", "perm-skill")
...
it.instance("defaultInfo returns resolved build agent when no default_agent config", () =>
  Effect.gen(function* () {
    ...
    expect(agent.name).toBe("operator")
