...
describe("configEntryNameFromPath", () => {
  test("strips an `agents/` prefix and returns the bare name", () => {
    expect(configEntryNameFromPath("agents/operator.md", AGENT_PREFIXES)).toBe("operator")
  })

  test("strips an `agent/` (singular) prefix", () => {
    expect(configEntryNameFromPath("agent/operator.md", AGENT_PREFIXES)).toBe("operator")
  })

  test("preserves nested subdirectories in the key", () => {
    expect(configEntryNameFromPath("agents/team/operator.md", AGENT_PREFIXES)).toBe("team/operator")
  })

  test("normalizes Windows-style backslashes", () => {
    expect(configEntryNameFromPath("agents\\team\\operator.md", AGENT_PREFIXES)).toBe("team/operator")
  })
  ...
  // the agent key (e.g. `.config/opencode/agents/operator`). Anchoring at the
  ...
  test("regression #25713: caller passes relative path; parent /agent/ segment is irrelevant", () => {
    ...
    const item = "/home/agent/.config/opencode/agents/operator.md"
    const relative = posix.relative(dir, item)
    expect(relative).toBe("agents/operator.md")
  ...
  test("regression #25713: parent /agents/ segment is irrelevant", () => {
    ...
    const item = "/srv/agents/team/.config/opencode/agents/operator.md"
    const relative = posix.relative(dir, item)
    expect(configEntryNameFromPath(relative, AGENT_PREFIXES)).toBe("operator")
  })
})
