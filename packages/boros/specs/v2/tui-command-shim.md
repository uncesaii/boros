...
Delete `packages/boros/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/boros/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/boros/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/boros`
- TUI plugin loader tests in `packages/boros` if runtime plugin API wiring changed
