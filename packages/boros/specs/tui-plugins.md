...
- Author package entrypoint is `@boros-ai/plugin/tui`.
...
- Import types from `@boros-ai/plugin/tui`.
- `@boros-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@boros-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `BOROS_CONFIG_DIR`, OpenCode installs `@boros-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@boros-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@boros-ai/plugin/tui`.
