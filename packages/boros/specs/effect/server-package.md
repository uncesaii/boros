...
- The server still lives in `packages/boros`.
...
Until enough shared service code lives outside `packages/boros`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/boros`

It should not import `packages/boros` services while `packages/boros`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/boros` implementation details.
