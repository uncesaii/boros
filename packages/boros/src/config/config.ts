...
import { InstallationLocal, InstallationVersion } from "@boros-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@boros-ai/core/v1/config/console-state"
import { FSUtil } from "@boros-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@boros-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@boros-ai/core/v1/config/config"
import { RemoteAuthError } from "@boros-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@boros-ai/core/v1/config/permission"
import { ConfigPluginV1 } from "@boros-ai/core/v1/config/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@boros/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["boros.jsonc", "boros.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "boros.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "opencode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "boros.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "BOROS_CONFIG_CONTENT") return "local"
        ...
        if (Flag.BOROS_CONFIG) {
          yield* merge(Flag.BOROS_CONFIG, yield* loadFile(Flag.BOROS_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.BOROS_CONFIG })
        }

        if (!Flag.BOROS_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.BOROS_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from BOROS_CONFIG_DIR", { path: Flag.BOROS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".boros") || dir === Flag.BOROS_CONFIG_DIR) {
            for (const file of ["boros.json", "boros.jsonc", "opencode.json", "opencode.jsonc"]) {
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["BOROS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("BOROS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.BOROS_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.BOROS_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("BOROS_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.BOROS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.BOROS_DISABLE_PRUNE) {
