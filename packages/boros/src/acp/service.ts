...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@boros-ai/sdk/v2"
...
export const AuthMethodID = "boros-login"
...
export class Service extends Context.Service<Service, Interface>()("@boros/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `boros auth login` in the terminal",
      name: "Login with boros",
      id: AuthMethodID,
    }
...
async function loadDirectorySnapshot(sdk: OpencodeClient, directory: string) {
  return ACPProfile.measure("acp.directory.load", async () => {
    ...
    return Directory.build({
      ...
      providers,
      modes,
      defaultModeID: agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ?? "operator",
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Boros service failure", service })
}
