...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@boros-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@boros-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@boros-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@boros-ai/tui/context/theme"
import { Global } from "@boros-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@boros-ai/core/util/flock"
import { Flag } from "@boros-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@boros-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@boros-ai/core/v1/config/plugin"
import { createCommandShim } from "@boros-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@boros-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".boros"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".boros", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".boros", "tui.json"),
  }
}
