...
```json
  "plugin": ["../packages/boros/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Boros server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/boros/script/run-workspace-server
```

With the plugin install, you can now run Boros and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
