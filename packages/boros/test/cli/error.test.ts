...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      ...
      },
      {
        ...
        data: { path: "/tmp/opencode.jsonc", dir: ".boros", suggestion: "opencode" },
        expected:
          'Directory ".boros" in /tmp/opencode.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `boros models` to list available models",
      "Or check your config (boros.json) provider/model names",
    ].join("\n")
