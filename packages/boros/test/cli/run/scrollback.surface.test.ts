...
import type { ToolPart } from "@boros-ai/sdk/v2"
...
test("turn summary starts at the left edge", async () => {
  ...
  try {
    await out.scrollback.writeTurnSummary({ agent: "Operator", model: "Little Frank", duration: "2.2s" })

    const commits = claim(out.renderer)
    try {
      expect(renderRows(commits.at(-1)!)[0]).toBe("▣ Operator · Little Frank · 2.2s")
    } finally {
      destroy(commits)
    }
  } finally {
