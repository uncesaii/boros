...
const original = {
  ...
  envPassword: process.env.BOROS_SERVER_PASSWORD,
  envUsername: process.env.BOROS_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.BOROS_SERVER_PASSWORD
  else process.env.BOROS_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.BOROS_SERVER_USERNAME
  else process.env.BOROS_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.BOROS_SERVER_PASSWORD = auth.password
  Flag.BOROS_SERVER_USERNAME = auth.username
  process.env.BOROS_SERVER_PASSWORD = auth.password
  process.env.BOROS_SERVER_USERNAME = auth.username
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-boros-directory": tmp.path },
      })
      expect(response.status).toBe(200)
