...
const originalWorkspaces = Flag.BOROS_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.BOROS_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.BOROS_EXPERIMENTAL_WORKSPACES = true
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "boros", "src")
