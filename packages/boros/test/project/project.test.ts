...
import { WorkspaceTable } from "@boros-ai/core/control-plane/workspace.sql"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const borosFile = path.join(tmp, ".git", "boros")
      expect(yield* Effect.promise(() => Bun.file(borosFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "boros")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "boros")
      const wrongCache = path.join(parentDir, ".git", "boros")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "boros")
      const cacheB = path.join(bareB, "boros")
      const wrongCache = path.join(parentDir, ".git", "boros")
