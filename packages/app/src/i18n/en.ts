...
export const dict = {
 ...
 "prompt.example.1": "Recon a target and map its attack surface",
 "prompt.example.2": "Enumerate open services and versions with nmap",
 "prompt.example.3": "Look up CVEs for the discovered service versions",
 "prompt.example.4": "Exploit a CVE for initial access",
 "prompt.example.5": "Escalate privileges to root",
 "prompt.example.6": "Web exploitation - SQLi, XSS, SSRF on the target",
 "prompt.example.7": "Capture a flag and prove the compromise",
 "prompt.example.8": "Research exploit techniques for the target stack",
 "prompt.example.9": "Analyze packets and leftovers for credentials",
 "prompt.example.10": "Password spraying against the login portal",
 "prompt.example.11": "Check for exposed admin panels and .git/.env leaks",
 "prompt.example.12": "Phishing awareness test on the domain",
 "prompt.example.13": "Decrypt a captured token or hash",
 "prompt.example.14": "Find lateral movement paths in Active Directory",
 "prompt.example.15": "Persist access and clean up artifacts",
 "prompt.example.16": "Build an exploit harness for the target service",
 "prompt.example.17": "Test the LLM/RAG app for prompt injection",
 "prompt.example.18": "Enumerate APIs and check for auth bypasses",
 "prompt.example.19": "Identify the tech stack before exploitation",
 "prompt.example.20": "Use websearch for recent exploit techniques",
