...
const boros = path.resolve(dir, "../../boros")

await $`bun dev generate > ${dir}/openapi.json`.cwd(boros)
...
// Patch a @hey-api/openapi-ts codegen bug: older versions incorrectly pass
// the endpoint's TError into the second generic of ServerSentEventsResult,
// which is the AsyncGenerator's TReturn slot. Iterator return values have
// nothing to do with HTTP errors, and any consumer that calls `.return()` or
// returns from a mock generator gets type-checked against the wrong shape.
// Drop the arg so TReturn defaults to void. Newer openapi-ts versions emit
// the fixed form directly (with the TError generic renamed `_TError` and
// unused), so the replace is a no-op there — accept either output.
