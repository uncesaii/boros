  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.agent.transform((draft) => {
      draft.update(AgentV2.defaultID, (item) => {
        item.description = "Boros root orchestrator — runs the offensive kill chain to a verified compromise by delegating to specialist operators."
        item.system ??= BOROS_ROOT + "\n\n" + BOROS_DOCTRINE
        item.mode = "primary"
      ...
      draft.update(AgentV2.ID.make("recon"), (item) => {
        item.description = "Reconnaissance operator — maps the attack surface and produces ranked exploitable leads with versions + vulnerability hypotheses."
      ...
      draft.update(AgentV2.ID.make("exploit"), (item) => {
        item.description = "Exploitation operator — turns weaknesses into real access (shells, flags, accounts) with evidence."
      ...
      draft.update(AgentV2.ID.make("privesc"), (item) => {
        item.description = "Privilege escalation operator — drives a foothold to root/SYSTEM/Domain Admin and proves it."
      ...
      draft.update(AgentV2.ID.make("web"), (item) => {
        item.description = "Web exploitation operator — breaks web apps/APIs (RCE/SQLi/SSRF/XSS/auth-bypass) with request/response evidence."
      ...
      draft.update(AgentV2.ID.make("triage"), (item) => {
        item.description = "Triage operator — classifies the objective by attacker intent and routes to the right specialist."
      ...
      draft.update(AgentV2.ID.make("assistant"), (item) => {
        item.description = "Assistant operator — tooling/payload dev, credential/foothold management, coordination, reporting."
      ...
      draft.update(AgentV2.ID.make("plan"), (item) => {
        ...
        item.permissions.push(
          ...PermissionV2.merge(defaults, [
            ...
            { action: "edit", resource: path.join(".boros", "plans", "*.md"), effect: "allow" },
            {
              action: "edit",
