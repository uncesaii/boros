...
import type { AssistantMessage, Part, Provider, UserMessage } from "@boros-ai/sdk/v2"
...
describe("transcript", () => {
  describe("formatAssistantHeader", () => {
    ...
    test("includes metadata when enabled", () => {
      ...
      expect(result).toBe("## Assistant (Operator · claude-sonnet-4-20250514 · 5.4s)\n\n")
    })

    test("uses model display name when available", () => {
      const result = formatAssistantHeader(baseMsg, true, providers)
      expect(result).toBe("## Assistant (Operator · Claude Sonnet 4 · 5.4s)\n\n")
    })
    ...
    test("handles missing completed time", () => {
      ...
      expect(result).toBe("## Assistant (Operator · claude-sonnet-4-20250514)\n\n")
    })
  ...
  describe("formatMessage", () => {
    ...
    test("formats assistant message with metadata", () => {
      ...
      const result = formatMessage(msg, parts, options)
      expect(result).toContain("## Assistant (Operator · Claude Sonnet 4 · 5.4s)")
  ...
  describe("formatTranscript", () => {
    test("formats complete transcript", () => {
      ...
      expect(result).toContain("## User")
      expect(result).toContain("Hello")
      expect(result).toContain("## Assistant (Operator · Claude Sonnet 4 · 0.5s)")
    ...
    test("falls back to raw model id when provider data is missing", () => {
      ...
      expect(result).toContain("## Assistant (Operator · claude-sonnet-4-20250514 · 0.5s)")
    })
