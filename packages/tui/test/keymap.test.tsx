...
import { getBorosModeStack, BOROS_BASE_MODE, BorosKeymapProvider, registerBorosKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerBorosKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when opencode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getBorosModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getBorosModeStack(keymap).push("autocomplete")
