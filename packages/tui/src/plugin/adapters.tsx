import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@boros-ai/plugin/tui"
...
import type { useBorosKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useBorosKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getBorosModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getBorosModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
