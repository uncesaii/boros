...
export const BOROS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const BOROS_MODE_KEY = "boros.mode"

export const BorosKeymapProvider = KeymapProvider
export const useBorosKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type BorosTuiKeymap = ReturnType<typeof useKeymap>
type OpencodeModeStack = ReturnType<typeof createBorosModeStack>
...
type Command = ReturnType<BorosTuiKeymap["getCommands"]>[number]
type BindingLookup = {
...
const modeStacks = new WeakMap<BorosTuiKeymap, OpencodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createBorosModeStack(keymap: BorosTuiKeymap) {
  keymap.setData(BOROS_MODE_KEY, BOROS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(BOROS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(BOROS_MODE_KEY, stack.at(-1)?.mode ?? BOROS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? BOROS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(BOROS_MODE_KEY, undefined)
...
export function useBorosModeStack() {
  return getBorosModeStack(useBorosKeymap())
}
...
function registerKeyAliases(keymap: BorosTuiKeymap) {
...
export function registerBorosKeymap(keymap: BorosTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createBorosModeStack(keymap)
...
export function useLeaderActive(): Accessor<boolean> {
  return useKeymapSelector((keymap: BorosTuiKeymap) => keymap.getPendingSequence()[0]?.tokenName === LEADER_TOKEN)
}

export function useCommandShortcut(command: string): Accessor<string> {
  const config = useTuiConfig()
  return useKeymapSelector((keymap: BorosTuiKeymap) =>
    formatKeySequence(
